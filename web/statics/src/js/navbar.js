        ...
        bind: function (callback, params) {
            ...
            _target.find('a[kit-target]').each(function () {
                ...
                _that.off('click').on('click', function () {
                    _that.closest("ul").find(".layui-this").removeClass("layui-this");
            ...
            $('.kit-side-fold').off('click').on('click', function () {
                ...
                if (_side.hasClass('kit-sided')) {
                    ...
                    _doc.find('iframe.layui-body').removeClass('kit-body-folded');
                    _doc.find('div.layui-footer').removeClass('kit-footer-folded');
                } else {
                    _side.addClass('kit-sided');
                    _doc.find('iframe.layui-body').addClass('kit-body-folded');
                    _doc.find('div.layui-footer').addClass('kit-footer-folded');
                }
//                alert($(this).width())
                $("#container").width($(window).width() - $(".layui-side-scroll").width() - 20);
            });
            return that;
        },
        /**
        ...
        render: function (callback) {
            var that = this,
                    ...
                    _tpl = [
                        ...
                        '<dd>',
                        '<a href="javascript:;" kit-target suri="" data-options="{url:\'../{{child.url}}\',icon:\'{{child.icon}}\',id:\'{{child.id}}\'}">',
                        ...
                        '</dl>',
                        '{{# }else{ }}',
                        '<a href="javascript:;" kit-target suri="" data-options="{url:\'{{item.url}}\',icon:\'{{item.icon}}\',id:\'{{item.id}}\'}">',
                        '{{# if (item.icon.indexOf("fa-") !== -1) { }}',
                        '<i class="fa {{item.icon}}" aria-hidden="true"></i>',
                        '{{# } else { }}',
                        '<i class="layui-icon">{{item.icon}}</i>',
