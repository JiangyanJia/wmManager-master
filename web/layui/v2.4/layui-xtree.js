...
layuiXtree.prototype.Initial = function (o) {
    ...
    if (o.color == null)
        o.color = {open: '#2F4056', close: '#2F4056', end: '#2F4056'};//图标颜色
    ...
    _this._ckbox = o.ckbox != null ? o.ckbox : true;  //全选是否启用
...
//            if (_this._ckbox)
            _this._domStr += '<input type="checkbox" class="layui-xtree-checkbox" ' + xtree_isend + xtree_ischecked + xtree_isdisabled + ' value="' + d[i].value + '" title="' + d[i].title+ '" id="' + d[i].value + '" grade="' + d[i].grade +'" ftype="' + d[i].ftype +'" pid="' + d[i].pid +'" lay-skin="primary" lay-filter="xtreeck' + _this._containerid + '">';
...
layuiXtree.prototype.Rendering = function () {
    ...
    if (!_this._ckbox) {
        $("div.layui-form-checkbox>i.layui-icon").hide();
    }
    ...
    for (var i = 0; i < xtree_items.length; i++) {
        if (xtree_items[i].parentNode == _this._container)
            ...
            xtree_items[i].style.margin = '5px 0 0 20px';
            if (!_this._isopen)
