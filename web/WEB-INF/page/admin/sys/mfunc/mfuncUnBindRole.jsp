    ...
    <body>
        <div class="x-left">
            <form class="layui-form" action="${BASE}/exp/user/save">
                ...
                <button class="layui-btn layui-btn-normal"  id="userwh" type='button'>解绑</button>
            </form>
            <table class="layui-table" id="bindList" lay-data="{url:'${BASE}/data/mfunc/mfuncUnBindRolelist?fid=${param.fid}&pid=${param.pid}',height: 'full-120', cellMinWidth: 80, page: true}"  lay-filter="bindList">
                <thead>
                    <tr>
        ...
        <script>
            layui.use(['laydate', 'table', 'form'], function () {
                ...
                form.on('submit(serch)', function (data) {
                    ...
                    table.reload('bindList', {method: "POST", where: data.field});
                    return false;
                });
                $("#userwh").on("click", function (e) {
                    ...
                    $.post("${BASE}/func/mfunc/unbindRole?fid=${param.fid}&pid=${param.pid}&rid=" + ulist, function (resp) {
                        if (resp.err == 0) {
                            layer.msg("解绑角色成功");
                            setTimeout(function () {
